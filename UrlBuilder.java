...
public class UrlBuilder {
    ...
    public static <T> String buildFullUrl(final EnvironmentProperties environmentProperties, final Class<T> type) {
        return Optional.ofNullable(type)
                .map(annotation -> type.getAnnotation(PartialUrl.class))
                .map(PartialUrl::value)
                .map(partialUrl -> String.format("%s/%s", environmentProperties.readProperty(), partialUrl))
    ...
    public static <T> String buildWholeUrl(final String commonUrl, final Class<T> type) {
        ...
        throw new IllegalStateException("Missing 'Partial Url' part!");
    }
}
